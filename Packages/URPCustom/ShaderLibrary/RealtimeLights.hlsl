...
{
    // Получаем линейное расстояние из квадрата расстояния
    float distance = sqrt(distanceSqr);

    // Получаем радиус действия света
    float range = rsqrt(distanceAndSpotAttenuation.x);

    // Нормализуем расстояние в диапазон от 0 до 1 (1 - у источника, 0 - на границе радиуса)
    float distance01 = saturate(1.0f - (distance / range));

    // Применяем квадратичный falloff. В этом и есть суть изменения!
    float lightAtten = pow(distance01, 2.0f);

    // Плавно сглаживаем затухание по всей дистанции
